        ...
        case EntryStatus.Fulfilled:
            {
                ...
                if (segment.isPartial && canNewFetchStrategyProvideMoreContent(segment.fetchStrategy, fetchStrategy)) {
                    ...
                    spawnedSegment = pingFullSegmentRevalidation(now, task, route, segment, tree, fetchStrategy);
                }
                break;
            }
        case EntryStatus.Pending:
...
function pingPerSegment(now, task, route, segment, routeKey, tree) {
    switch(segment.status){
        case EntryStatus.Empty:
            // Upgrade to Pending so we know there's already a request in progress
...
function pingPPRSegmentRevalidation(now, task, currentSegment, route, routeKey, tree) {
    ...
    switch(revalidatingSegment.status){
        case EntryStatus.Empty:
            ...
            // upon completion.
            upsertSegmentOnCompletion(task, route, tree.cacheKey, spawnPrefetchSubtask(fetchSegmentOnCacheMiss(route, upgradeToPendingSegment(revalidatingSegment, FetchStrategy.PPR), routeKey, tree)));
            break;
        case EntryStatus.Pending:
            break;
        case EntryStatus.Fulfilled:
...
function pingFullSegmentRevalidation(now, task, route, currentSegment, tree, fetchStrategy) {
    const revalidatingSegment = readOrCreateRevalidatingSegmentEntry(now, currentSegment);
    if (revalidatingSegment.status === EntryStatus.Empty) {
        ...
        upsertSegmentOnCompletion(task, route, tree.cacheKey, waitForSegmentCacheEntry(pendingSegment));
...
const noop = ()=>{};
function upsertSegmentOnCompletion(task, route, cacheKey, promise) {
