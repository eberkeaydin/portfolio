...
export function getSegmentKeypathForTask(task, route, cacheKey) {
    ...
    const isDynamicTask = task.fetchStrategy === FetchStrategy.Full || task.fetchStrategy === FetchStrategy.PPRRuntime || !route.isPPREnabled;
...
export function readSegmentCacheEntry(now, route, cacheKey) {
...
export function requestOptimisticRouteCacheEntry(now, requestedUrl, nextUrl) {
    ...
    const routeWithNoSearchParams = readRouteCacheEntry(now, createPrefetchRequestKey(requestedUrl.origin + requestedUrl.pathname, nextUrl));
 ...
 * entry, If not, it adds an empty entry to the cache and returns it.
 */ export function readOrCreateSegmentCacheEntry(now, task, route, cacheKey) {
